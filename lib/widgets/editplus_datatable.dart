...
class _EditPlusDataTableState extends State<EditPlusDataTable> {
            ...
            children: [
              ...
              ElevatedButton.icon(
                onPressed: () {
                  ...
                  tableWidthScrollOffset =
                      (tableWidthScrollOffset < maxScrollExtent)
                          ? (tableWidthScrollOffset +
                              min(
                                50.0,
  ...
  List<DataRow> getDataRows(var dataRowsMapList) {
    ...
    for (var dataRowContent in dataRowsMapList) {
      ...
      var dataRow = DataRow(
        cells: dataCells,
        key: getRowKeyValue(dataRowContent),
      ); // ValueKey<String>(dataRowContent[widget.keyColumnName].toString()));
                  ...
                  TextButton(
                    onPressed: () {
                      ...
                      ValueKey rowValueKey =
                          dataRowToSave.key as ValueKey<String>;
  ...
  DataCell getDataCell(var columnName, var cellValue, int rowIndex) {
    if (rowEditIndex == -1) {
      if (widget.fieldsWithEditors != null &&
          widget.fieldsWithEditors!.contains(columnName)) {
        return DataCell(widget.getFieldEditorFunction!(columnName, cellValue));
      }

      // return DataCell(Text(cellValue));
      return DataCell(
        getTextFormField(columnName, rowIndex, false, dataValue: cellValue),
      );
    } else if (rowEditIndex == rowIndex) {
      return DataCell(
        getTextFormField(columnName, rowIndex, true, dataValue: cellValue),
      );
    }
  ...
  bool getCreateState() {
    return widget.tableEditStatus['CREATINGROW'] == null
        ? false
        : widget.tableEditStatus['CREATINGROW'];
  }

  bool getEditState() {
    return widget.tableEditStatus['EDITINGROW'] == null
        ? false
        : widget.tableEditStatus['EDITINGROW'];
  }
  ...
  TextFormField getTextFormField(
    ...
    Function textFieldFormatFunction =
        (widget.fieldsWithFormattingFunction != null &&
                widget.fieldsWithFormattingFunction!.containsKey(textName) &&
                widget.fieldsWithFormattingFunction![textName] is Function)
    ...
    for (var dataRow in dataRows) {
      ...
      for (var columnName in widget.columnNames) {
        ...
        if (drcell.child is Text) {
          ...
          EditPlusStringDropdown cellEntry =
              drcell.child as EditPlusStringDropdown;
          mpDrow[columnName] = cellEntry.valueContainer['VALUE'];
